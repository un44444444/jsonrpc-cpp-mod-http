 ...
 * \brief Simple JSON-RPC HTTP server.
...
{
  TestRpc a;
  Json::Rpc::HttpServer server(std::string("0.0.0.0"), 8086);
  ...
  server.AddMethod(new Json::Rpc::RpcMethod<TestRpc>(a, &TestRpc::Print, std::string("print_self")));
  ...
  std::cout << "Start JSON-RPC HTTP server" << std::endl;
  ...
  std::cout << "Stop JSON-RPC HTTP server" << std::endl;
  server.Close();
  networking::cleanup();
